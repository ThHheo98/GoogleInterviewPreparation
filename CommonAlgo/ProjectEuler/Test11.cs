...
namespace CommonAlgo.ProjectEuler
{
    public class Test11
    {
        ...
        public void Test()
        {
            ...
            // check diag from up left to down right
            for (int i = 3; i < 20; i++)
            {
                for (int j = 0; j <= i - 3; j++)
                {
                    //under diag
                    int localMax = a[i - j - 0][j + 0]
                                   *a[i - j - 1][j + 1]
                                   *a[i - j - 2][j + 2]
                                   *a[i - j - 3][j + 3];
                    if (localMax > max)
