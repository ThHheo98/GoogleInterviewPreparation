...
Here is another example. If a number appears 5 times at most, 
we can write a program using the same method. Now we need 3 bits and the loop is 000->100->010->110->001. The code looks like this:
...
I hope all these above can help you have a better understand of this problem.
        ...
        public int singleNumber(List<int> aList)
        {
            int ones = 0, twos = 0;
            for (var i = 0;
                i < aList.Count;
                i++)
            {
                ones = (ones ^ aList[i]) & ~twos;
                twos = (twos ^ aList[i]) & ~ones;
            }
            return ones;
        }
    }
